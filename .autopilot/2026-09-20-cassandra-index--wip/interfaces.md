...
- Известные оговорки: поддеревья скопированы, не вынесены в reusable-компоненты; тренд в макете
  — столбцы (линия с точками формат не поддерживает); `"version": "2.6"` — как в образце,
  схема расширения 0.6.71 объявляет 2.17 — проверить открытие в pen.dev на приёмке
