...
- `calc.js` не вызывает `engine.regionalIndex` (входы без региональной атрибуции,
  воспроизведена только ветка слепых регионов nReg=0) — при появлении regions во входах
  расширять calc.js через `engine.regionalIndex`.
- `calc.js --write` самопроверку делает ПОСЛЕ записи: провал контракта отклоняет результат,
  но диск уже перезаписан.
...
`node --test` без аргументов (20 файлов, 150 passed / 0 fail — подтверждено оркестратором).
